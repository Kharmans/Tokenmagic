import './proto/ProgramProto.js';

const _tempRect = new PIXI.Rectangle();
export function CustomFilterMixin(FilterClass) {
	return class extends FilterClass {
		constructor({ vertexSrc, fragmentSrc, args }) {
			super(...args);
			this.uniforms.filterMatrix ??= new PIXI.Matrix();
			this.uniforms.filterMatrixInverse ??= new PIXI.Matrix();

			if (vertexSrc || fragmentSrc) {
				this.program = PIXI.Program.from(
					vertexSrc ?? this.program._originalVertexSrc,
					fragmentSrc ?? this.program._originalFragmentSrc,
				);
			}
		}
